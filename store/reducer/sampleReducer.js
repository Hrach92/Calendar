import { count, dayCount, hoursOfDay, months, years } from "../../Components/Instance";
...
const initialState = {
    ...
    dayCount:dayCount,
    notes:[{img:'/google.png',title:'Keep',id:1},{img:'/notes.png',title:'Notes',id:2},{img:'/contact.png',title:'Contacts',id:3},{img:'/google-maps.png',title:'Maps',id:4}],
    events:{
        dayEvents:[],
        hourEvents:[]
    },
...
export const Features = createSlice({
    ...
    reducers:{
        ...
        addEvents:(state,action) =>{
            ...
            let hourEvents = action.payload.filter(({hour_mode})=>hour_mode===true)
            state.events = {
                dayEvents:(arr.length>0?[...dayEvents]:[]),
                hourEvents:(hourEvents.length>0?[...hourEvents]:[])}
        },
        getLocaleDate:(state,action) =>{
            state.month = {...state.months.find(v=>v.id===(new Date().getMonth()+1))};
...
export const {setMonth,setMode,setYear,setDay,addEvents,getLocaleDate,
    getEventDate,setNotesOpen,setTimeRange,
    setColor,setColors,setHourMode} = Features.actions
